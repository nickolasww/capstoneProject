  ...
  return (
    <div className="flex min-h-[80vh] flex-col items-center justify-center px-4 py-20 text-center relative overflow-hidden">
      ...
      <div className="relative mb-12">
        <div className="flex h-32 w-32 items-center justify-center rounded-3xl bg-linear-to-br from-green-50 to-green-100 shadow-xl border border-green-200/50 rotate-3 transition-transform hover:rotate-6">
          <Construction className="h-16 w-16 text-[#4D9232]" />
        </div>
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight text-gray-900 leading-tight">
          Layanan <span className="text-[#4D9232]">{title}</span> <br className="hidden md:block" />
          Sedang Disiapkan
        </h1>
      ...
      <div className="mt-12 flex flex-wrap items-center justify-center gap-4">
        <Link
          to="/"
          className="flex items-center gap-2 rounded-xl bg-[#4D9232] px-8 py-3.5 font-bold text-white shadow-lg shadow-green-200 transition-all hover:bg-[#3d7527] hover:shadow-green-300 active:scale-95 group"
        >
      ...
      <div className="mt-20 flex flex-col items-center gap-6">
        <div className="flex items-center justify-center gap-3">
          {[...Array(3)].map((_, i) => (
            <div 
              key={i} 
              className={`h-1.5 rounded-full bg-[#4D9232] transition-all duration-1000 animate-pulse`} 
              style={{ width: `${(i + 1) * 20}px`, animationDelay: `${i * 200}ms` }}
            ></div>
          ))}
        </div>
        <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#4D9232]/60">
          <div className="h-1 w-1 rounded-full bg-[#4D9232]/60"></div>
          Under Construction
          <div className="h-1 w-1 rounded-full bg-[#4D9232]/60"></div>
        </div>
      </div>
